...
def compensation(hist_actual, hist_pred, pred, j, period=7, steps_day=7):
    ...
    if len(deviation) <= 3:
        ...
        # SES
        # 拟合历史数据
        fit_SES = SimpleExpSmoothing(deviation).fit(smoothing_level=1/10, initial_level=np.average(deviation, weights=weights))  # 有时fit_SES_train训练出的smoothing_level会比较大，fit_SES的smoothing_level设置小些，使预测更趋于中部。
        fit_SES_train = SimpleExpSmoothing(deviation).fit(optimized=True, use_brute=False, start_params=np.array([1/10]), method='Powell', initial_level=np.average(deviation, weights=weights))
        # 打印模型参数
        ...
        # Holt
        # fit models
        Holt_add_dam = Holt(deviation, exponential=False, damped_trend=True).fit(smoothing_level=1/5, smoothing_trend=1/5,
            damping_trend=0.90, initial_level=np.average(deviation, weights=weights),
        ...
        Holt_add_dam_train = Holt(deviation, exponential=False, damped=True).fit(optimized=True, damping_trend=0.95,
            use_brute=False, start_params=np.array([1/5, 1/5]), method='TNC', initial_level=np.average(deviation, weights=weights),
            initial_trend=np.array((sum(deviation[int(np.ceil(len(deviation) / 2)):]) - sum(deviation[:int(np.floor(len(deviation) / 2))])) / (np.floor(len(deviation) / 2)) ** 2))
        # print parameters
        print('Hlot_[4,14]:')
        ...
        # Holt
        # fit models
        Holt_add_dam = Holt(deviation, exponential=False, damped_trend=True).fit(smoothing_level=1 / 5,
                                                                                 smoothing_trend=1 / 5,
                                                                                 damping_trend=0.90,  optimized=False,
        ...
        HW_add_add_dam_train.forecast(steps_day).rename('HW_add_add_dam_train').plot(ax=ax_HoltWinters, color='r', legend=True)
        ...
        # print internals
        df_HW_add_add_dam = pd.DataFrame(np.c_[deviation, HW_add_add_dam.level, HW_add_add_dam.slope, HW_add_add_dam.season, HW_add_add_dam.fittedvalues],
        ...
        HW_add_add_dam_residual = (np.array(HW_add_add_dam.forecast(steps_day)) - np.array(pred)) / np.array(pred) * 100  # pd.Series是按对应索引进行运算，np.array是按对应顺序进行运算
        ...
        HW_add_add_dam_train_residual = (np.array(HW_add_add_dam_train.forecast(steps_day)) - np.array(pred)) / np.array(pred) * 100  # pd.Series是按对应索引进行运算，np.array是按对应顺序进行运算
...
if __name__ == "__main__":
    for i in range(len(length)-1):
        final_pred_add = compensation(hist_actual=list(y_input_add_actual[i][:3]), hist_pred=np.array(y_input_add_pred[i][:3]),
        ...
        # print('final prediction:')
        # print(final_pred_add, '\n')
        final_pred_add = compensation(hist_actual=y_input_add_actual[i][:-steps_day], hist_pred=y_input_add_pred[i][:-steps_day],
